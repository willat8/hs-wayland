...
getEncodersStatus = do
    req <- parseRequest "http://rancher:6544/Dvr/GetEncoderList" >>= \req -> return req { requestHeaders = [("Accept", "application/json")], responseTimeout = Just 1000000 }
...
getChannelIcon path = do
    bsreq <- parseRequest ("http://rancher:6544" ++ path) >>= \req -> return req { responseTimeout = Just 1000000 }
