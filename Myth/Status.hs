...
getEncodersStatus = do
    req <- parseRequest "http://192.168.0.65/Dvr/GetEncoderList" >>= \req -> return req { requestHeaders = [("Accept", "application/json")], responseTimeout = Just 5000000 }
...
getChannelIcon path = getUrl ("http://192.168.0.65" ++ path)
