...
drawLittleText win_w win_h ss = do
    ...
    XP.setFontSize 30
    ys <- zipWith (\i (XP.TextExtents _ yb _ h _ _) -> win_h -  i * (h - yb + 5)) [1..] <$> mapM XP.textExtents lines
    zipWithM_ (\y s -> XP.setSourceRGBA 1 0.2 0.2 0.6
                    >> XP.arc (win_w / 5 - 20) (y - 5) 10 0 (fromIntegral 2 * pi)
                    >> XP.fill
...
statusCheck t_ptr _ = do
    ...
    peek status_ptr >>= \status -> poke status_ptr status { statusNumEncoders = (fromIntegral $ length encoders), statusEncoders = encoders, statusShowClock = (encoders == statusEncoders status) }
...
redrawHandler _ d_ptr = do
    ...
    Status _ window_ptr _ w h _ _ show_clock _ encoders <- peek status_ptr
...
statusCreate display_ptr w h = do
    mallocForeignPtr >>= \status_fp -> withForeignPtr status_fp $ \status_ptr -> do
        FC.addForeignPtrFinalizer status_fp $ peek status_ptr >>= \(Status _ window_ptr widget_ptr _ _ check_fd _ _ _ _) -> do
        ...
        poke status_ptr (Status display_ptr window_ptr widget_ptr w h check_fd check_task True 0 [])
