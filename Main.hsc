...
alertCheck t_ptr _ = do
    ...
    Alert widget_ptr check_fd _ _ _ _ _ _ <- peek alert_ptr
    fdRead check_fd #{size uint64_t}
    babyMonitorHealth <- getBabyMonitorStatus
    isHDHomeRunHealthy <- getHDHomeRunStatus
    peek alert_ptr >>= \alert -> poke alert_ptr alert { alertBabyMonitorHealth = babyMonitorHealth, alertHDHomeRunHealth = isHDHomeRunHealthy }
...
alertHide t_ptr _ = do
    ...
    Alert widget_ptr _ _ hide_fd _ _ _ _ <- peek alert_ptr
...
alertResizeHandler _ _ _ d_ptr = do
    Alert widget_ptr _ _ _ _ _ _ _ <- peek (castPtr d_ptr)
    c_widget_set_allocation widget_ptr 0 0 800 80

alertRedrawHandler _ d_ptr = do
    Alert widget_ptr _ _ _ _ babyMonitorHealth isHDHomeRunHealthy showDashboard <- peek (castPtr d_ptr)
    xp <- c_widget_cairo_create widget_ptr
    xpsurface <- XP.mkSurface =<< c_cairo_get_target xp
    c_cairo_destroy xp
    drawAlert xpsurface showDashboard babyMonitorHealth isHDHomeRunHealthy =<< c_widget_get_last_time widget_ptr
    unless (babyMonitorHealth == healthy && isHDHomeRunHealthy) $ c_widget_schedule_redraw widget_ptr
